...
const blinder = (function () {
  ...
  const oneColorAllDebug = createCommonjsModule(function (module, exports) {
    /*jshint evil:true, onevar:false*/
    const installedColorSpaces: string[] = [];

    let namedColors = {};

    const undef = function (obj) {
      return typeof obj === 'undefined';
    };
    const channelRegExp = /\s*(\.\d+|\d+(?:\.\d+)?)(%)?\s*/;
    const percentageChannelRegExp = /\s*(\.\d+|100|\d?\d(?:\.\d+)?)%\s*/;
    const alphaChannelRegExp = /\s*(\.\d+|\d+(?:\.\d+)?)\s*/;
    const cssColorRegExp = new RegExp(
      '^(rgb|hsl|hsv)a?' +
        '\\(' +
    ...
    function ONECOLOR(obj) {
      if (Object.prototype.toString.apply(obj) === '[object Array]') {
        if (
          typeof obj[0] === 'string' &&
          typeof ONECOLOR[obj[0]] === 'function'
        ) {
        ...
        const lowerCased = obj.toLowerCase();
        ...
        const matchCssSyntax = obj.match(cssColorRegExp);

        if (matchCssSyntax) {
          const colorSpaceName = matchCssSyntax[1].toUpperCase();
          const alpha = undef(matchCssSyntax[8])
            ? matchCssSyntax[8]
            : parseFloat(matchCssSyntax[8]);
          const hasHue = colorSpaceName[0] === 'H';
          const firstChannelDivisor = matchCssSyntax[3]
            ? 100
            : hasHue
              ? 360
              : 255;
          const secondChannelDivisor = matchCssSyntax[5] || hasHue ? 100 : 255;
          const thirdChannelDivisor = matchCssSyntax[7] || hasHue ? 100 : 255;

          if (undef(ONECOLOR[colorSpaceName])) {
            throw new Error(
              'one.color.' + colorSpaceName + ' is not installed.'
            );
          }
    ...
    function installColorSpace(colorSpaceName, propertyNames, config) {
      ...
      const prototype = ONECOLOR[colorSpaceName].prototype;
      ...
      prototype.equals = function (otherColor, epsilon) {
        ...
        for (let i = 0; i < propertyNames.length; i = i + 1) {
          if (
            Math.abs(
      ...
      for (const propertyName in config) {
        ...
        if (config.hasOwnProperty(propertyName)) {
          const matchFromColorSpace = propertyName.match(/^from(.*)$/);
      ...
      function installForeignMethods(
        ...
        ONECOLOR[sourceColorSpaceName].propertyNames.forEach(function (
          propertyName,
          i
        ) {
    ...
    installColorSpace('RGB', ['red', 'green', 'blue', 'alpha'], {
      ...
      hexa: function () {
        const alphaString = Math.round(this._alpha * 255).toString(16);
    ...
    installColorSpace('XYZ', ['x', 'y', 'z', 'alpha'], {
      fromRgb: function () {
        ...
        const convert = function (channel) {
          return channel > 0.04045
            ? Math.pow((channel + 0.055) / 1.055, 2.4)
            : channel / 12.92;
        };
        const r = convert(this._red);
        const g = convert(this._green);
        const b = convert(this._blue);
      ...
      lab: function () {
        ...
        const x = convert(this._x / 95.047);
        const y = convert(this._y / 100.0);
        const z = convert(this._z / 108.883);
    ...
    installColorSpace('LAB', ['l', 'a', 'b', 'alpha'], {
      ...
      xyz: function () {
        ...
        const convert = function (channel) {
          const pow = Math.pow(channel, 3);

          return pow > 0.008856 ? pow : (channel - 16 / 116) / 7.87;
        };
        const y = (this._l + 16) / 116;
        const x = this._a / 500 + y;
        const z = y - this._b / 200;
    ...
    installColorSpace('HSV', ['hue', 'saturation', 'value', 'alpha'], {
      ...
      hsl: function () {
        const l = (2 - this._saturation) * this._value;
        const sv = this._saturation * this._value;
        const svDivisor = l <= 1 ? l : 2 - l;
    ...
    installColorSpace('HSL', ['hue', 'saturation', 'lightness', 'alpha'], {
      hsv: function () {
        ...
        const l = this._lightness * 2;
        const s = this._saturation * (l <= 1 ? l : 2 - l);
    ...
    function gs() {
      ...
      // @ts-ignore
      const rgb = this.rgb();
      const val = rgb._red * 0.3 + rgb._green * 0.59 + rgb._blue * 0.11;
    ...
    ONECOLOR.installMethod('mix', function (otherColor, weight) {
      ...
      // eslint-disable-next-line @typescript-eslint/ban-ts-comment
      // @ts-ignore
      const a = this._alpha - otherColor._alpha;
      const weight1 = ((w * a === -1 ? w : (w + a) / (1 + w * a)) + 1) / 2;
...
*/
    ONECOLOR.installMethod('toAlpha', function (color) {
      ...
      // @ts-ignore
      const me = this.rgb();
      const other = ONECOLOR(color).rgb();
      const epsilon = 1e-10;
      // eslint-disable-next-line @typescript-eslint/ban-ts-comment
      // @ts-ignore
      const a = new ONECOLOR.RGB(0, 0, 0, me._alpha);
      ...
      channels.forEach(function (channel) {
        me[channel] =
          (me[channel] - other[channel]) / me._alpha + other[channel];
      });
      me._alpha *= a._alpha;
...
*/
  ...
  const convertRgbToXyz = function (o) {
    const M = matrixRgbXyz;
    const z = {};
  ...
  const convertXyzToXyy = function (o) {
    const n = o.X + o.Y + o.Z;
  ...
  const colorBlind = createCommonjsModule(function (module, exports) {
    /*
     * color-blind
     * https://github.com/skratchdot/color-blind
     *
    ...
    const createBlinder = function (key) {
      return function (colorString, returnRgb) {
        const color = oneColorAllDebug(colorString);
    ...
    for (const key in colorVisionData) {
